...
def export_pdf(buffer,pajak):
    ...
    outlet.alamat_outlet FROM barang_keluar 
    INNER JOIN detail_sales on detail_sales.id = barang_keluar.id_sales 
    INNER JOIN sales on sales.id = detail_sales.id_sales 
    INNER JOIN outlet on outlet.id = detail_sales.id_outlet WHERE barang_keluar.id = %s """, (id,))
    barang_keluar = g.con.fetchone()
    
    g.con.execute("SELECT barang.nama_barang, jmlpermintaan, barang.qty, harga_satuan, harga_total, diskon " \
    ...
    for i in range(len(data_db)):
            ...
            if data_db[i][5] and data_db[i][5] != '' and data_db[i][5] != 0:
               ada_diskon = True
    pdf = canvas.Canvas(buffer, pagesize=(21.6 * cm, 14.5 * cm))  # ukuran custom 21.6cm x 14.4cm (A4)
    ...
    pdf.drawString(x_margin_9 + 4 * cm, y2, "Kepada Yth:")
    ...
    alamat_lines = textwrap.wrap(alamat, width=40)
    pdf.drawString(x_margin_9 + 4 * cm, y2 - 0.4 * cm, barang_keluar[4])  # Nama outlet
    for i, line in enumerate(alamat_lines[:11]):  # Limit to 6 lines
        pdf.drawString(x_margin_9 + 4 * cm, y2 - (0.8 + i * 0.4) * cm, line)  # Alamat outlet

    # Tabel barang
    pdf.setFont("Helvetica-Bold", 9)
    if ada_diskon == True:
        headers = ["No", "Nama Barang", "Qty", "Harga", "Diskon", "Total"]
    else:
        headers = ["No", "Nama Barang", "Qty", "Harga","Total"]
    header_positions = [x_margin, x_margin + 2 * cm, x_margin + 9 * cm, x_margin + 11 * cm, x_margin + 14 * cm, x_margin + 15.5 * cm]
    ...
    for i in range(12):
        if y < 2 * cm:
            ...
            y = height - 2 * cm
        if i < len(data_db):
            nama_barang, jml, satuan, harga_satuan, total, diskon = data_db[i]
            qty = f"{jml} {satuan}"
            if ada_diskon == True:
                values = [str(i + 1), nama_barang, qty, f"Rp{harga_satuan:,.0f}",f"{diskon}%", f"Rp{total:,.0f}"]
            else:
                values = [str(i + 1), nama_barang, qty, f"Rp{harga_satuan:,.0f}", f"Rp{total:,.0f}"]
    ...
    if pajak == "pajak":
        ...
        pdf.drawString(x_margin + 16 * cm, y, f"Rp{jumlah_total:,.0f}")
        pajak = (11 / 100) * jumlah_total  
        print(pajak)
        pdf.drawString(x_margin + 14 * cm, y - 0.4 * cm, f"PPN 11%: ")
        pdf.drawString(x_margin + 16 * cm, y - 0.4 * cm, f"Rp{pajak:,.0f}")
        grand_total = jumlah_total + pajak
        print(grand_total)
        terbilang_rupiah = num2words(int(grand_total), lang='id')
        terbilang_rupiah = terbilang_rupiah.title() 
        terbilang_rupiah += " Rupiah"
        pdf.setFont("Helvetica-Bold", 8)
        print(len(terbilang_rupiah))
        if len(terbilang_rupiah) >= 90:
            terbilang_rupiah_lines = textwrap.wrap(terbilang_rupiah, width=90)
            for i, line in enumerate(terbilang_rupiah_lines[:3]):  # Limit to 6 lines
                pdf.drawString(x_margin ,y - (i * 0.4) * cm, line)  # terbilang_rupiah
        else:
            pdf.drawString(x_margin, y - 0.8 * cm, terbilang_rupiah)
        pdf.drawString(x_margin + 14 * cm, y - 0.8 * cm, f"Grand Total: ")
        pdf.drawString(x_margin + 16 * cm, y - 0.8 * cm, f"Rp{grand_total:,.0f}")

    else:
        pdf.setFont("Helvetica-Bold", 10)
        terbilang_rupiah = num2words(jumlah_total, lang='id')
        terbilang_rupiah = terbilang_rupiah.title() 
        pdf.drawString(x_margin, y, f"{terbilang_rupiah} Rupiah")
        pdf.drawString(x_margin + 14 * cm, y, f"Total: Rp{jumlah_total:,.0f}")
    y -= 1.4 * cm

    # Footer
    pdf.drawString(x_margin, y, "Penerima")
    pdf.drawString(x_margin + 7 * cm, y, "Gudang")
    pdf.drawString(x_margin + 14 * cm, y, "Expedisi")
...
def annual_report():
    ...
    c.drawCentredString(width / 2, height - 3 * cm, f"Tahun: {datetime.now().year}")
    ...
    for row in ringkasan:
        if y < 3 * cm:
            c.showPage()
            y = height - 3 * cm
        c.drawString(2 * cm, y, f"""{row['nama_barang'][:30]:30} | Beli: {row['total_pembelian']} 
        | Jual: {row['total_penjualan']} | Stok: {row['stok_akhir']} | Laba: Rp {row['total_laba']:,.2f}""")
        y -= 0.5 * cm

    c.setFont("Helvetica", 10)
    total_pendapatan = sum([row[1] for row in ringkasan])
    total_pengeluaran = sum([row[0] for row in ringkasan])
    laba_bersih = sum([row[3] for row in ringkasan])
    ...
    c.drawString(2 * cm, 2 * cm, f"Dibuat pada {datetime.now().strftime('%d-%m-%Y')}")
